...
def main():
	...
	while True:
		...
		result1 = json.loads(html_content)
		if not flag:
			html = get_one_page(page)
			html_content = get_real_content(html)
			...
			db = pymysql.connect(host='localhost', port=3306, user='root', password='123456', database='spider',
								 charset='utf8')
			cursor = db.cursor()
			for item in items:
